...
### Fork selection
Fork selection refers to how nodes select best fork for them to append new block to. Here fork is defined as a path from root node to one of the leaf node. In some implementations this selection process can be greedy, where nodes select fork which maximize their rewards. Best fork must contain all finalized blocks.
